...
function describe (reply, fallback) {
  ...
  return { alive: true, pid: reply.pid ?? null, peers: reply.peers ?? 0, uptime: reply.uptime ?? 0 }
}
...
async function runRelay (dir, opts = {}) {
  const { log = console, swarm = true, say = null } = opts
  ...
  try {
    ...
    node = new Relay(store, {
      ...
      onPeers: (peers) => {
        writeState(dir, { pid: process.pid, peers, startedAt })
    ...
    joining = swarm === false
      ? Promise.resolve()
      : node.start().then(
        () => log.log('[relay] joined the topic, waiting for peers'),
...
class Relay {
  constructor (store, { onChange = noop, onPeers = noop, swarm = null } = {}) {
    this.store = store
  ...
  // force skips the unannounce round trip, which is what a process on its way
  // out wants: DHT records expire on their own and nobody should wait for it.
  // The race is the backstop, because destroy has no timeout of its own.
  async stop ({ force = true, timeout = 2000 } = {}) {
