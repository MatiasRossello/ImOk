...
async function local (dir, opts) {
  ...
  const client = {
    ...
    async close () {
      ...
      // Same shape the socket path reports. Dropping reach and bluetooth here
      // made the first run after a boot claim the machine had no radio.
      client.relay = {
        alive: started.alive,
        pid: started.pid,
        peers: started.peers,
        reach: started.reach ?? null,
        bluetooth: started.bluetooth ?? null,
