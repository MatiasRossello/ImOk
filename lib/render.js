...
const DEFAULT_LIMIT = 10 // rows drawn by `list`, overridden with --limit
...
// The wordmark that opens a check-in, one per state. Two of them rather than
// one: this block sits directly above the state line, and an "I'M OK" drawn
// over "I need help." would argue with itself on the screen.
// ponytail: art by hand, held to 4 rows and under MIN_COLUMNS by a test;
// a font table would be more code than six letters are worth
const BANNERS = {
  ok: [
...
// Cyan for ok, amber for help: the two colours the rest of the interface
// already gives those states.
function banner (status) {
  const colour = status === 'alert' ? style.amber : style.cyan
  return (BANNERS[status] ?? BANNERS.ok).map((line) => paint(colour, line))
}

function checkIn ({ status, peers, reach = null, gateways = 0 }) {
  const lines = ['']
  lines.push(...banner(status))
  lines.push('')
  lines.push(bold(status === 'alert' ? paint(style.amber, 'I need help.') : "I'm ok."))
  lines.push('')
...
// limit caps the rows drawn, not what the device holds: the footer keeps
// counting everything, because how much you are carrying for other people is
// the number this whole program exists to show. null draws them all.
function roster (messages, { columns = DEFAULT_COLUMNS, now = Date.now(), query = null, peers = 0, limit = DEFAULT_LIMIT } = {}) {
  const width = clampColumns(columns)

  if (messages.length === 0) return emptyRoster({ query, peers })

  // already newest first out of the store, so the newest are the ones kept
  const shown = limit === null ? messages : messages.slice(0, limit)
  ...
  const rows = shown.map((message) => {
    const state = message.status === 'alert'
      ? paint(style.amber, pad('▲ help', statusWidth))
      : dim(pad('● ok', statusWidth))
    return [
  ...
  return ['', dim(header), ...rows, '', dim(footer(messages.length, peers, shown.length)), ''].join('\n')
}
...
function footer (total, peers, shown = total) {
  const held = shown < total
    ? `last ${shown} of ${total} ${plural(total, 'check-in')}`
    : `${total} ${plural(total, 'check-in')}`
  return `${held} · carried for ${peers} ${plural(peers, 'peer')} in range`
}
...
function me ({ id, profile, stats, relay, version = null }) {
  ...
  lines.push(paint(style.cyan, 'This is an OTA update DEMO'))
  lines.push('')
  return lines.join('\n')
}
