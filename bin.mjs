...
const cmd = command(
  ...
  flag('--relay', 'run the relay in the foreground').hide(),
...
console.log(`\nYou are ${shortId(identity.publicKey)}`)
console.log(`Storage: ${dir}\n`)

if (cmd.flags.relay) await runRelay(dir, identity, cmd.flags.say)
...
async function runRelay (dir, identity, note) {
  const store = await openStore(dir, { publicKey: identity.publicKey })
  await store.purge()

  if (note) {
    const msg = create({ name: shortId(identity.publicKey), status: 'ok', note, zone: 'test' }, identity)
    console.log(`[store] ${await store.put(msg)}: ${note}`)
  }

  const relay = new Relay(store, {
    onChange: (msg) => console.log(`[recv] ${msg.name} (${msg.pk.slice(0, 8)}): ${msg.note}`)
  })
  await relay.start()
  ...
  const timer = setInterval(() => {
    const s = store.stats()
    console.log(`[stat] peers=${relay.peers} total=${s.total} mine=${s.mine} others=${s.others}`)
  }, 2000)
